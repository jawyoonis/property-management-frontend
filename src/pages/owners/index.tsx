...
const emptyOwner: OwnerDto = { firstName: '', lastName: '', email: '', phone: '' };
...
export default function OwnersPage() {
  ...
  const [form, setForm] = useState<OwnerDto>(emptyOwner);
  ...
  const openCreate = () => { setEditing(null); setForm(emptyOwner); setModalOpen(true); };
  const openEdit = (o: OwnerDto) => { setEditing(o); setForm({ ...o }); setModalOpen(true); };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editing?.ownerId) {
        await ownerService.update(editing.ownerId, form);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
            <Users className="w-7 h-7 text-blue-600" /> Property Owners
          </h1>
          <p className="text-gray-500 text-sm mt-1">{owners.length} total owners</p>
        </div>
        <button
          onClick={openCreate}
              ...
              {filtered.map(owner => (
                <tr key={owner.ownerId} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-gray-600">
                    <div className="flex items-center gap-2"><Mail className="w-4 h-4 text-gray-400" />{owner.email}</div>
                  </td>
                  <td className="px-6 py-4 text-gray-600">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => openEdit(owner)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
                        <Edit2 className="w-4 h-4" />
                      </button>
                      <button onClick={() => owner.ownerId && handleDelete(owner.ownerId)} className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors">
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Owner' : 'Add New Owner'}>
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button type="submit" disabled={saving}
              className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium disabled:opacity-60">
